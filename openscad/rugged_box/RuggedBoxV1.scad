      ...
      translate([0, -boxLengthYMm, 0]) // add back in the rim width
        ...
        for (h = [1:numberOfLatches]) {
          ...
          actualLatchCenterOffsetMm =
            h < (numberOfLatches + 1) / 2 ? -latchCenterOffsetMm
            : h == (numberOfLatches + 1) / 2 ? 0
            : latchCenterOffsetMm;
...
module BotomStandardHinge() {
  ...
  for (h = [1:numberOfHinges]) {
    ...
    actualHingeCenterOffsetMm =
      h < (numberOfHinges + 1) / 2 ? -hingeCenterOffsetMm
      : h == (numberOfHinges + 1) / 2 ? 0
      : hingeCenterOffsetMm;
...
module BoxShellBase(height, additionalShellThickness = 0) {
  ...
  for (x = [1:numSideSupportRibs]) {
    actualRibCenterOffsetMm =
      x < (numSideSupportRibs + 1) / 2 ? -ribCenterOffsetMm
      : x == (numSideSupportRibs + 1) / 2 ? 0
      : ribCenterOffsetMm;
...
module Wall2D(wallThickness, additionalWallThickness, height, isRimIncluded, isForRotation) {
  ...
  difference() {
    ...
    for (cnt = [0:additionCount]) {
      additionXAdj =
        (cnt == additionCount && ( (additionalWallThickness % wallThickness) != 0)) ? ( (cnt - 1) * wallThickness) + (additionalWallThickness % wallThickness)
        : cnt * wallThickness;
